...
export class GreenhouseAutoApplyBot {
  ...
  constructor(options: BotOptions = {}) {
    ...
    this.resumePath = options.resumePath || process.env.RESUME_PATH || './MacAndersonUcheCVAB.pdf';
    this.coverLetterPath = options.coverLetterPath || process.env.COVER_LETTER_PATH || './cover-letter.txt';
    this.config = this.loadConfig();
    // Store browser context in a local directory
    this.contextPath = options.contextPath || path.join(process.cwd(), '.browser-context');
    this.jobsSearchUrl = options.jobsSearchUrl || process.env.JOBS_SEARCH_URL || `${this.baseURL}/jobs?query=software%20engineer&date_posted=past_five_days&work_type[]=remote`;
    ...
    if (openaiApiKey) {
      ...
      console.warn('⚠️  OPENAI_API_KEY not found. Text box auto-fill will be disabled.');
    }
  }
  ...
  async login(): Promise<boolean> {
    ...
    if (!this.headless) {
      ...
      console.log('⏳ Waiting for authentication (headless mode)...');
    }
  ...
  async searchJobs(shouldNavigate: boolean = true): Promise<Job[]> {
    ...
    if (shouldNavigate) {
      ...
      console.log(`   📍 Using search URL: ${this.jobsSearchUrl}`);
      await this.page.goto(this.jobsSearchUrl, {
        waitUntil: 'networkidle',
      });
   ...
   */
  async applyToJobByUrl(jobUrl: string, jobTitle: string = 'Unknown Job'): Promise<boolean> {
    if (!this.page || !this.context) {
  ...
  async saveFailedSubmissions(): Promise<void> {
    try {
      const failedSubmissionsPath = path.join(process.cwd(), 'failed-submissions.json');
      const data = {
...
Generate an appropriate answer for this job application field based on the resume information provided.`;
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      // Keep browser open for a bit so user can see results (only if not headless)
      if (!this.headless) {
